<template>
    <h1 class="unselectable">This is under construction!<br>The cows in the <i>cow food delivery area</i> are hungry! Drag the food to them!</h1>

    <div>
        <button style="margin-bottom: 1em; margin-top: 1em;" class="button-35" @click="removeMovableObject">Careful! This button will delete the cow food.</button>
        <div  style="margin-bottom: 3em;" class="unselectable" id="food-area"><span style="color: #fff; ">Cow food delivery area</span></div>
        
        <h1 class="unselectable delivered-color" v-if="isCollisionDetected">{{ collisionMessage }} </h1>
        <h1 class="unselectable undelivered-color" v-else>Cow Food not delivered!</h1>
        

    <div style="padding-top: 1em;" class="text-container">
        <span class="unselectable" style="font-size: 12px; color: #fff;">
            This array below is tracking whether the food is already in the cow delivery area, or it is outside the cow delivery
            area. This prevents any winning message from showing up when the food is already inside the cow food delivery area.<br>
            <span style="font-size: 2em;">{{ collisionCount }}</span>
            <br>
            Number of times the cow food has entered the cow food delivery area: <span style="font-size: 2em;">{{ cowFoodDeliveryCount }}</span>
            <br>
            Number of times cow food has left the cow delivery area: <span style="font-size: 2em;">{{ cowFoodExitsDeliveryAreaCount }}</span>
        </span>
    </div>
        <div id="movable-object" :style="{ top: position.top + 'px', left: position.left + 'px' }" @mousedown="startDrag"><b
                class="unselectable" style="color: white; pointer-events: none;">Cow Food</b></div>
    </div>

    <h1 class="unselectable" v-if="winningMessage">The cows say Moo!<br>They are happy!<br>You win!</h1>
...
export default {
    ...
    methods: {
        removeMovableObject(event) {
            const movableObject = document.getElementById("movable-object");
            if (movableObject && !movableObject.contains(event.target)) {
